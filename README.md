...
proving persistence of scheme and field reconstruction from admissible multi-fundamental-group data through a multi-lane anabelian reconstruction program super-architecture.

## Main Manuscript

- [paper/ANABELIAN_RECONSTRUCTION_PROGRAM_PREPRINT.md](paper/ANABELIAN_RECONSTRUCTION_PROGRAM_PREPRINT.md)
...
1. section, birational, and packet-reconstruction packages,
...
- `paper/`
  - `ANABELIAN_RECONSTRUCTION_PROGRAM_PREPRINT.md`
...
1. Super-lane theorem chain first: read `paper/ANABELIAN_RECONSTRUCTION_PROGRAM_PREPRINT.md`.
...
Current ARP runner policy:

- `repro/run_repro.sh` extracts, promotes, runs `scripts/arp_closure_guard.py`, updates `repro/repro_manifest.json`, and enforces `fully_extracted` release-gate mode.
...
3. artifact key consumed by `scripts/arp_closure_guard.py`.
...
- Manuscript target: [paper/ANABELIAN_RECONSTRUCTION_PROGRAM_PREPRINT.md](paper/ANABELIAN_RECONSTRUCTION_PROGRAM_PREPRINT.md)
