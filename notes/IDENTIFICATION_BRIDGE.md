...
In-paper anchor: `paper/ANABELIAN_RECONSTRUCTION_PROGRAM_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of scheme and field reconstruction from admissible multi-fundamental-group data through a multi-lane anabelian reconstruction program super-architecture`.
...
4. the strict closure margin `M_ARP` remains positive.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_ARP` is meaningful.
