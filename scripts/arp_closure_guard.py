...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['ARP_G1', 'ARP_G2', 'ARP_G3', 'ARP_G4', 'ARP_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['ARP_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['ARP_GM'] = 'PASS' if gm_pass else 'FAIL'
    blockers = {}
    ...
    blockers['ARP_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['ARP_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    normalized_gates = {'G1': gates['ARP_G1'], 'G2': gates['ARP_G2'], 'G3': gates['ARP_G3'], 'G4': gates['ARP_G4'], 'G5': gates['ARP_G5'], 'G6': 'NA', 'GCoh': gates['ARP_G6'], 'GM': gates['ARP_GM']}
    normalized_blockers = {'G1': blockers['ARP_G1'], 'G2': blockers['ARP_G2'], 'G3': blockers['ARP_G3'], 'G4': blockers['ARP_G4'], 'G5': blockers['ARP_G5'], 'G6': [], 'GCoh': blockers['ARP_G6'], 'GM': blockers['ARP_GM']}
